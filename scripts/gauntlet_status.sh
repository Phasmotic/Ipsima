...
talaria_classify_tier_b_final() {
    talaria_classification_reset
    local view_rc="${1-}" final_record="${2-}" head_sha="${3-}"
    local expected_title="${4-}" watch_rc="${5-}"
    local final_status="" final_conclusion="" final_sha="" final_title=""

    if [ "$#" -ne 5 ] \
        || ! talaria_is_exit_code "$view_rc" || ((10#$view_rc != 0)) \
        || [ -z "$final_record" ]; then
        TALARIA_CLASS_DETAIL="could not verify the final Tier B run state"
        return 0
    fi
    if ! talaria_is_exit_code "$watch_rc" \
        || ! talaria_is_commit_sha "$head_sha" \
        || [[ ! "$expected_title" =~ ^Talaria\ Tier\ B:\ talaria-[0-9a-f]{32}$ ]] \
        || ! talaria_is_single_line "$final_record" \
        || [[ ! "$final_record" =~ ^([^|]*)\|([^|]*)\|([^|]*)\|([^|]+)$ ]]; then
        TALARIA_CLASS_DETAIL="could not verify the final Tier B run state"
        return 0
    fi
    final_status="${BASH_REMATCH[1]}"
    final_conclusion="${BASH_REMATCH[2]}"
    final_sha="${BASH_REMATCH[3]}"
    final_title="${BASH_REMATCH[4]}"
    if ! talaria_is_commit_sha "$final_sha"; then
        TALARIA_CLASS_DETAIL="could not verify the final Tier B run state"
    elif [ "$final_sha" != "$head_sha" ]; then
        TALARIA_CLASS_DETAIL="the completed Tier B run reports a different commit"
    elif [ "$final_title" != "$expected_title" ]; then
        TALARIA_CLASS_DETAIL="the completed Tier B run reports a different correlation title"
    elif [ "$final_status" != "completed" ]; then
        TALARIA_CLASS_DETAIL="Tier B observation ended before the run completed (watch rc=$watch_rc)"
    else
    ...
    TALARIA_CLASS_VALUE="$final_sha"
    TALARIA_CLASS_VALUE2="$final_conclusion"
}
